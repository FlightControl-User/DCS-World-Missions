...
function Transport_Reload( HeliGroup, CountryPrefix, CargoShip )
  ...
  if CountryPrefix == 'RU' then
    ...
    if CargoShipGroup:IsAlive() then
      local OrbitTask  = HeliGroup:TaskOrbitCircleAtVec2( CargoShipGroup:GetVec2(), 10, 0 )
...
--- @param Wrapper.Group#GROUP SU34Group
...
do -- USA Deploy troops to battle zone
  ...
  CargoTable[CargoItem] = AI_CARGO_GROUP:New( 'Engineers', 'Engineer ' .. EngineerNames[CargoItem],
...
do -- Russia Rescue workers from oil platforms
  ...
  CargoTable[CargoItem] = AI_CARGO_GROUP:New( 'Oil workers', 'Worker ' .. WorkerNames[CargoItem],
