...
do -- DESIGNATE for BLUE
  ...
  -- With the MISSION object US_M1, which is the main mission.
  M1_DesignateBlue = DESIGNATE:New( US_CC, US_A2G_Detection, US_A2G_Attack, US_M1 )

  -- For the SU-25T, setup laser code 1113.
  M1_DesignateBlue:GenerateLaserCodes()

  -- This makes the DESIGNATE appear in the menu under the mission named "Designate for SU-25T".
  M1_DesignateBlue:AddMenuLaserCode( 1113, "Lase for Su-25T laser code %d" )
  M1_DesignateBlue:AddMenuLaserCode( 1680, "Lase for A-10A laser code %d" )
...
do -- GCICAP for RED
  ...
  AG_AI_A2A_Dispatcher:SetDefaultTakeoffFromParkingCold()
  ...
  -- Set a strong overhead. We want intruders to be punished immediately!
  AG_AI_A2A_Dispatcher:SetDefaultOverhead( 1.5 )
  
  -- Set a grouping of 3. So if 4 planes are spawned they attack in a group.
  AG_AI_A2A_Dispatcher:SetDefaultGrouping( 3 )
  
  -- We set a specific grouping for the Tonopah Airport airbase, so CAP departing from Tonapah Airport flies in groups of 2 planes.
  AG_AI_A2A_Dispatcher:SetSquadronGrouping( AIRBASE.Nevada.Tonopah_Airport, 2 )
...
do -- GCICAP for BLUE
  ...
  -- Change the default takeoff methods.
  US_A2A_Dispatcher:SetDefaultTakeoffInAir()
  US_A2A_Dispatcher:SetDefaultLandingNearAirbase()
  ...
  US_AWACS_Spawn = SPAWN
    ...
    :OnSpawnGroup(
      function( AwacsGroup )
        ...
        US_A2A_Dispatcher:SetSquadronCapInterval( "Creech AFB", 2, 60, 120 )  
      end
    )
...
do -- AIRBASE POLICE
  ...
  SetClient = SET_CLIENT:FilterCoalitions( coalition.side.BLUE ):FilterStart()

  US_Airbase_Police = AIRBASEPOLICE_NEVADA:New( SetClient )
